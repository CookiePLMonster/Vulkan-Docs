...
For any pname:pSwapchains member of pname:pPresentInfo, if a slink: is
provided in the pNext chain with a non-zero pname:presentStageQueries, one
entry of its internal timing queue is reserved until the results have been
collected by flink:vkGetPastPresentationTimingEXT.
