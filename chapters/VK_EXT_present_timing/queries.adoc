...
include::{generated}/api/protos/vkGetPastPresentationTimingEXT.adoc[]
  ...
  * pname:pTimingPropertiesChanged is `NULL` or a pointer to a
    basetype:VkBool32, which is set to ename:VK_TRUE if the swapchain timing
    properties have changed since the last call to
    fname:vkGetPastPresentationTimingEXT.
...
Results for specific outstanding presentation requests can: be queried by
setting the pname:presentId field of input pname:pPresentationTimings
entries. If pname:timeout is not zero, fname:vkGetPastPresentationTimingEXT
will block for that many nanoseconds or until all the results for the
specified present ids are available. If the timeout expires,
ename:VK_TIMEOUT is returned and no result is written to
pname:pPresentationTimings. If pname:timeout is code:UINT64_MAX, the timeout
